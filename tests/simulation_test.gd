...
func _test_unit_food_costs(failures: Array[String]) -> void:
	...
	if simulation.command_train(stronghold_id, &"worker"):
	...
	if not simulation.command_train(stronghold_id, &"worker"):
...
func _test_food_building(
	...
	if site.x < 0:
		...
		return
	if not simulation.command_build(worker_id, structure_kind, site):
...
func _test_ai_food_economy(failures: Array[String]) -> void:
	...
	simulation._ai_income_timer = 0.0
...
func _test_faction_food_traditions(failures: Array[String]) -> void:
	...
	for faction in expectations:
		...
		if bool(expected["farm"]) == bool(expected["hunt"]):
			continue
		if simulation.command_build(worker_id, blocked_kind, MapCatalog.PLAYER_BUILD_TEST_SITE):
	...
	if not human_simulation.command_train(lodge_id, &"hunter"):
	...
	if celestial_simulation.command_train(forbidden_lodge_id, &"hunter"):
...
func _run() -> void:
	...
	if workers.size() != 3:
		...
		var deposited_workers := simulation.command_deposit([workers[0]], player_stronghold_id)
	...
	if cave_ids.size() != 2:
		...
		if simulation.command_train(cave_ids[0], &"jadeclaw"):
		...
		hunter["cell"] = Vector2i((hunter["position"] as Vector2).round())
		simulation.command_stop([hunter_id])
		...
		if int(cave.get("team", RtsSimulation.TEAM_NEUTRAL)) != RtsSimulation.TEAM_PLAYER:
			...
			if not simulation.command_train(cave_ids[0], &"jadeclaw"):
				...
				if player_jadeclaws.size() <= jadeclaws_before:
					...
					if not simulation.command_train(cave_ids[0], &"jadeclaw"):
						...
						simulation.command_stop([rival_capturer_id])
						simulation._ai_strategy_timer = 999.0
						_advance(simulation, RtsSimulation.CAVE_CAPTURE_SECONDS + 1.0)
	...
	if tree_id < 0:
		...
		simulation.command_gather([workers[0]], tree_id)
	...
	if not simulation.command_build_war_camp(workers[0], build_site):
		...
		if not camp.is_empty() and float(camp.get("complete", 0.0)) >= 1.0 and not simulation.command_train(camp_id, &"vanguard"):
