...
func _test_shift_order_queue(failures: Array[String]) -> void:
	...
	simulation.command_move([worker_id], first)
	simulation.command_move([worker_id], second, false, true)
	simulation.command_move([worker_id], origin)
	if not (worker.get("command_queue", []) as Array).is_empty():
		failures.append("a replacing move did not clear queued orders")
	simulation.command_move([worker_id], first, false, true)
	simulation.command_stop([worker_id])
	if worker.get("order") != &"idle" or not (worker.get("command_queue", []) as Array).is_empty():
...
func _test_patrol(failures: Array[String]) -> void:
	...
	if not simulation.command_patrol([unit_id], destination):
...
func _test_repair(failures: Array[String]) -> void:
	...
	if not simulation.command_repair([worker_id], stronghold_id):
	...
	simulation.command_repair([worker_id], stronghold_id)
	...
	if simulation.command_repair([worker_id], enemy_hold):
...
func _test_training_cancellation(failures: Array[String]) -> void:
	...
	if not simulation.command_train(camp_id, &"vanguard"):
		failures.append("Vanguard could not be queued for cancellation test")
		return
	if not simulation.command_train(camp_id, &"mystic"):
	...
	var cancelled := simulation.command_cancel_training(camp_id, RtsSimulation.TEAM_PLAYER)
	...
	if not simulation.command_cancel_training(camp_id, RtsSimulation.TEAM_ENEMY).is_empty():
...
func _test_modifier_input_forwarding(failures: Array[String]) -> void:
	...
	simulation.command_move([vanguard_id], first)
	var second_screen := (
	...
	simulation.command_stop([vanguard_id])
	simulation.command_move([vanguard_id], first)
