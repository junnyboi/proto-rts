...
func command_attack(ids: Array[int], target_id: int, append: bool = false) -> void:
...
func command_gather(ids: Array[int], resource_id: int, append: bool = false) -> void:
	...
	for id in ids:
		...
		if (
			...
			continue
		_issue_unit_order(worker, {"type": &"gather", "target_id": resource_id}, append)
	_add_event(&"command", _entity_center(resource), Color("73e6bc"))


func command_deposit(ids: Array[int], stronghold_id: int, append: bool = false) -> int:
...
func command_repair(ids: Array[int], target_id: int, append: bool = false) -> bool:
	var target := entity(target_id)
	var issued := false
	for id in ids:
		var worker := entity(id)
...
func command_patrol(ids: Array[int], destination: Vector2i, append: bool = false) -> bool:
	var units: Array[Dictionary] = []
	for id in ids:
		var unit := entity(id)
		if _is_commandable_unit(unit) and _is_military_unit(unit):
			units.append(unit)
	if units.is_empty():
		return false
	var formation := _formation_cells(destination, units.size())
...
func command_cancel_training(structure_id: int, requesting_team: int) -> Dictionary:
	var structure := entity(structure_id)
	if (
		structure.is_empty()
...
func _advance_attack_order(attacker: Dictionary, delta: float) -> bool:
	...
	if target.is_empty() or not bool(target.get("alive", false)) or not target_is_visible:
		...
		if attacker.get("order") == &"patrol":
			var patrol_acquired := _nearest_enemy(attacker, float(attacker["acquire_range"]))
	...
	if needs_cave and army.size() >= 3:
		...
		if not player_hold.is_empty():
			...
			if is_entity_visible_to_team(TEAM_ENEMY, player_hold):
				...
				command_move(ids, player_hold["cell"] as Vector2i, true)
...
func _try_train_ai_hunters() -> void:
	...
	for lodge in _team_structures_of_kind(TEAM_ENEMY, &"hunters_lodge"):
		...
		if hunter_total >= 2:
			return
		if command_train(int(lodge["id"]), &"hunter"):
			hunter_total += 1
