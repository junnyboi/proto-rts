...
The shipped shell blocks result-screen input, and the simulation independently enforces the terminal boundary. Every validated public gameplay mutation uses `_can_issue_command(...)`, which rejects commands after `outcome` or from eliminated teams. A terminal outcome also stops the current combat iteration, aborts later tick phases, discards residual fixed-step time, and prevents subsequent ticks. New public gameplay mutators must use the same guard and return their normal failure value without changing state or emitting events.
...
**Navigation topology.** A* contains static walkability plus every living structure/resource footprint, including incomplete foundations; live units, wildlife, and the Dragon Egg are deliberately absent. Construction placement and resource/structure death rebuild the graph and increment its revision. Preserve both A* and runtime diagonal no-corner-cut checks. Friendly ordinary structures and gates are relaxed only for their owner's path calculation; friendly walls/towers and all enemy structures remain solid. An Egg carrier is the deliberate exception and routes around every structure so the synchronized Egg never enters a footprint. Friendly pass-through never removes line-of-sight blocking.
...
The Egg is unattackable and unlocks when guardian Shenlong dies. Multiple empty Workers may race claim orders; the first to interaction range wins. Claim starts a return-home order, but the carrier remains normally commandable and cannot gather. Carrier death, Stronghold loss, or elimination drops the Egg onto a nearby walkable cell. While alive, the Egg's current one-cell footprint is reserved from every structure and wall-line placement, whether guarded, unlocked, carried, or dropped; the reservation follows its synchronized cell and disappears after hatching. It is intentionally absent from A* and line-of-sight blockers so units can still reach and escort it.
...
The title-screen EN/CN selector applies for the current app session. It intentionally has no durable locale preference yet. `ThemeFactory` installs the original CC0 ManusCC0 Regular face for UI text, with bundled Noto Sans CJK SC as the fallback. ManusCC0 Medium and Bold are also available with matching advances. Keep the CJK copyright notice with its font. The economy ribbon measures complete counters and reflows columns to fit the available width. `tests/localization_test.gd` checks catalog/key/placeholder parity, literal call-site keys, tweak descriptor keys, raw-copy regressions, title switching, and catalog-wide font coverage; `tests/localization_visual_capture.gd` produces nine Chinese UI states, including tutorial, generated tweak, pause, and confirmation surfaces, for manual layout review.[15]
...
`LeaderboardStore` writes `user://mandate_of_myth_leaderboard.json` with schema version 1. Local aggregates track every result, while parallel ranked aggregates track only eligible matches, victories, best score, last faction, and update time. A run row stores ID, match number, score, result, faction, elapsed seconds, completion time, ranking eligibility, and a bounded run-configuration marker. Only the ranked aggregates are exposed to the optional global bridge. The newest 30 rows are retained independently of lifetime aggregates. Simulation score breakdown and per-match resource/kill/build/capture/repair statistics are not persisted.[9]

The profile ID must be 32 lowercase hexadecimal characters. Callsigns use Chinese characters, ASCII letters/numbers, spaces, hyphens, or underscores, with combining marks attached to letters. Names containing Chinese require at least 2 Unicode codepoints; other names retain the 3-codepoint minimum. All names are trimmed and capped at 20 codepoints. Invalid schema, profile ID, or callsign invalidates the whole file; invalid run rows are dropped. Scores clamp to 0–9,000,000,000. The store rotates a backup, restores a valid `.bak` after primary corruption, and creates a fresh profile only when neither validates. Local rank is score descending, then elapsed time ascending, completion time ascending, and run ID.
...
The Web preset is single-threaded and excludes build outputs, captures, docs, tests, tools, provenance reports, the duplicate authoring CJK font, and two unused textures. The runtime copy now contains the approved 992,160-byte ManusGameSC Common subset for dynamic names. It retains all current gameplay and unchanged retained art/audio. High-resolution source masters stay excluded only while **`assets/source/.gdignore` is preserved**. Matching Godot 4.7.2 Web templates are required.[1] [10] See [the optimization results and delivery runbook](docs/lean-delivery.md) for measured savings, Brotli, package profiles, and preservation evidence.
